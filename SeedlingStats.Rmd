...
Welch t test
Use when: means of two groups with unequal sample sizes and possible unequal variances
Assumptions: normality of data

```{r}
hist(Childs06_2024$Diameter)

t.test(Childs06_2024$Diameter, Childs10_2023$Diameter, var.equal = FALSE)
...
Use when: You have more than two groups with unequal sample sizes, and you want to compare their means.
Example: Comparing the average height of seedlings across multiple treatment groups, each with a different number of samples.
Why: ANOVA can handle unequal sample sizes but assumes equal variances across groups (you can relax this assumption with a modified version like Welch’s ANOVA).
